...
data9 = df[name[8]] #Open BID
data10 = df[name[9]] #High BID
data11 = df[name[10]] #Low BID
data12 = df[name[11]] #Close BID
...
for ts in range(0,data1len):
    ...
    if '=' in data1[ts] :
        print (_dateTime1.strftime('%Y-%m-%dT%H:%M:%S.000Z') + ',' + data1[ts] + ',' + str(data9[ts]) + ',' + str(data10[ts]) + ',' + str(data11[ts]) + ',' + str(data12[ts]), flush=True)
    else:
